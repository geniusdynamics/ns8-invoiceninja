...
reponame="invoiceninja"

# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-invoiceninja container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-invoiceninja; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-invoiceninja -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/mariadb:10.11.5 docker.io/invoiceninja/invoiceninja:5" \
    "${container}"
# Commit the image
